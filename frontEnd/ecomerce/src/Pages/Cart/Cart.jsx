    ...
    <div className='pt-4'>
                ...
                <tr>
                  <td>
                    <div className='flex p-4'>
                      <img src={item?.thumbnail} alt={item?.name + " image"} className='w-[120px] h-[120px] object-cover'/>
                  ...
                  <td>
                    <NumberInput quantity={item?.quantity} min={1} max={item?.variant?.stockQuantity} onChangeQuantity={(value) => onChangeQuantity(value, item?.productId, item?.variant?.id)}></NumberInput>
                  </td>
                  <td>
      ...
      <div className='flex justify-between bg-gray-200 p-8'>
        ...
        <div className='mr-20 pr-8'>
          ...
          {!isLoggedIn && <div className='p-4'><Link to={"/v1/login"} className='w-full p-2 items-center h-[48px] bg-black border rounded-lg mt-2 text-white hover:bg-gray-800' onClick={() => navigate("/checkout")}>Login to Checkout</Link></div>}
        </div>
      </div></>}
      {
        !cartItems?.length &&
